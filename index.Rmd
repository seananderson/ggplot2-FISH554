...
*February 2016, by Sean C. Anderson*
...
Work your way through this page. When you want to check your work, or if you get really stuck on an exercise, just click on the button `R source` and you'll see the R source code that created a figure.
...
```{r aes-example, bootstrap.show.code=FALSE}
...
```{r facet1, fig.height=9, fig.width=9, bootstrap.show.code=FALSE}
ggplot(morph, aes(wingl, beakh)) +
...
```{r facet2, fig.heigh=9, fig.width=9, bootstrap.show.code=FALSE}
ggplot(morph, aes(wingl, beakh)) +
...
```{r facet3, fig.width=16, fig.height=3, bootstrap.show.code=FALSE}
ggplot(morph, aes(wingl, beakh)) +
...
```{r loess-example, bootstrap.show.code=FALSE}
ggplot(morph, aes(wingl, beakh)) +
...
:   ggvis is a partial successor to ggplot2 and is under rapid development: https://github.com/rstudio/ggvis. It focuses on implementing a similar grammar of graphics for web-based and possibly interactive output. Install it with `devtools::install_github("rstudio/ggvis")`.
